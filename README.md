...
| 檔案 | 說明 |
|------|------|
| `train_yolo11.py` | 訓練腳本：支援 ISAT / YOLO 格式，自動轉換並訓練 detect / segment 模型 |
| `TRAINING_GUIDE.md` | 訓練完整指南：ISAT 標注 → 資料集轉換 → 訓練 → TRT 部署 |
| `yolov11_tensorrt_jetson.py` | 推理腳本：PT → ONNX → TRT FP16 三段推理流程 |
...
使用自有資料集訓練自訂類別的偵測 / 分割模型，完整流程詳見 [TRAINING_GUIDE.md](TRAINING_GUIDE.md)。

### 快速訓練指令

```bash
# ISAT 標注資料（自動轉換格式，預設 segment）
...
轉換後的資料集輸出至 `<source>_yolo_converted/`，**原始 ISAT 資料不會被修改**。

訓練完成的 `best.pt` 即可接後續 TRT export 流程。
